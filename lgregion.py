...
# ---------------- CONFIG ----------------
WINDOW_NAME = "PokeMМO"
# List of OCR regions: (x, y, width, height)
# Original region: Top left (140, 175) to bottom right (330, 190)
regions = [
    (140, 175, 190, 15),  # original
    (398, 111, 190, 15),  # added per request
    (553, 130, 190, 15),  # added per request
]
MOVE_DELAY = 0.5
E_PRESS_DELAY = 0.25
OCR_PSM = "--psm 7"
...
def ocr_worker():
    while not stop_flag.is_set():
        # Capture and OCR all configured regions, then combine results
        texts = []
        for reg in regions:
            img = capture_window(hwnd, reg)
            texts.append(ocr_region_to_text(img))
        ...
        if matched:
            ...
            if matched.lower() in LEGENDARIES_LOWER:
                ...
                while not stop_flag.is_set():
                    ...
                    # Capture all regions again during battle and combine
                    texts2 = []
                    for reg2 in regions:
                        img2 = capture_window(hwnd, reg2)
                        texts2.append(ocr_region_to_text(img2))
                    text2 = " ".join(t for t in texts2 if t)
